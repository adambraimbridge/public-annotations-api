...
func (cd cypherDriver) read(contentUUID string) (anns annotations, found bool, err error) {
	...
	query := &neoism.CypherQuery{
		Statement: `
			...
			WITH c, collect({id: cc.uuid, predicate: type(rel), types: labels(cc), prefLabel: cc.prefLabel, leiCode: i.value, prefUUID: canonical.prefUUID, canonicalPrefLabel: canonical.prefLabel, canonicalLEI: canonical.LEICode, canonicalTypes: labels(canonical), lifecycle:rel.lifecycle}) + rows as moreRows
			...
			WITH collect({id: cd.uuid, predicate:'IS_CLASSIFIED_BY', types: labels(cd), prefLabel: cd.prefLabel,leiCode: null, canonicalPrefLabel: canon.prefLabel, prefUUID: canon.prefUUID, canonicalTypes: labels(canon), canonicalLEI: null, lifecycle:rel.lifecycle}) + moreRows as allRows
			UNWIND allRows as row
			WITH DISTINCT(row) as drow
			RETURN distinct drow.id as id, drow.predicate as predicate, drow.types as types, drow.prefLabel as prefLabel, drow.leiCode as leiCode, drow.canonicalPrefLabel as canonicalPrefLabel, drow.prefUUID as prefUUID, drow.canonicalTypes as canonicalTypes,  drow.canonicalLEI as canonicalLei, drow.lifecycle as lifecycle
			`,
	...
	return predicateFilter.FilterAnnotations(mappedAnnotations), found, nil
}
...
func (cd cypherDriver) filteredRead(contentUUID string, platformVersion string) (anns annotations, found bool, err error) {
	...
	query := &neoism.CypherQuery{
		Statement: `
			...
			WITH c, collect({id: cc.uuid, predicate: type(rel), types: labels(cc), prefLabel:cc.prefLabel, uuids:upp, tmeIDs:tme, leiCode:lei.value, factsetID:fs, platformVersion:rel.platformVersion}, lifecycle:rel.lifecycle) as rows
			UNWIND rows as row
			WITH DISTINCT(row) as drow
			RETURN drow.id as id, drow.predicate as predicate, drow.types as types, drow.prefLabel as prefLabel, drow.leiCode as leiCode, drow.factsetID as factsetID, drow.uuids as uuids, drow.tmeIDs as tmeIDs, drow.platformVersion as platformVersion, drow.lifecycle as lifcycle
			`,
