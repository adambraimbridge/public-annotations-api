...
func main() {
	...
	app.Action = func() {
		...
		if *env != "local" {
			f, err := os.OpenFile("/var/log/apps/public-annotations-api-go-app.log", os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0755)
			if err == nil {
				log.SetOutput(f)
...
func runServer(neoURL string, port string, cacheDuration string, env string) {
	...
	servicesRouter.HandleFunc("/ping", annotations.Ping)
	servicesRouter.HandleFunc("/__ping", annotations.Ping)
	...
	http.HandleFunc("/__build-info", annotations.BuildInfoHandler)
	http.HandleFunc("/build-info", annotations.BuildInfoHandler)
